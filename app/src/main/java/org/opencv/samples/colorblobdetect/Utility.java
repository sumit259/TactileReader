...
//        boolean result = true;          // wrong point. reverses the purpose of this method
        /*for (int a = 0; a < points.size(); a++) {
            Log.i("POLYGON_TEST", "Points " + a + ": " + points.get(a).x + "  " + points.get(a).y);
        }*/
        for (i = 0, j = points.size() - 1; i < points.size(); j = i++) {
//            Log.i("POLYGON_TEST", "for loop i = " + i + " j = " + j);
            if ((points.get(i).y > test.y) != (points.get(j).y > test.y) &&
                    (test.x <
                            (points.get(j).x - points.get(i).x) * (test.y - points.get(i).y) / (points.get(j).y - points.get(i).y) + points.get(i).x)) {
                result = !result;
